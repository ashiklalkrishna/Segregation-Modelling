...
to setup
  ...
  ask patches [
    set pcolor white
    if random-float 100 < density-of-agents [   ; control overall occupancy %
      ifelse random-float 100 < density-of-red-agents [
        sprout 1 [
          set color red
          set size 1
        ]
...
to compute-moran
  let reds count turtles with [color = red]
  let n count turtles
  if n = 0 [ set moranI 0 stop ] ; avoid division by zero
  ...
  ask turtles [
    let xi (ifelse-value (color = red) [1] [0])
    let diff_i (xi - xbar)
    set denominator denominator + (diff_i ^ 2)

    ask neighbors with [any? turtles-here] [
      let xj mean [ifelse-value (color = red) [1] [0]] of turtles-here
      let diff_j (xj - xbar)
      set numerator numerator + (diff_i * diff_j)
      set W W + 1
    ]
  ]

  ifelse denominator != 0 and W != 0 [
    set moranI (n / W) * (numerator / denominator)
  ] [
    set moranI 0
  ]
end
...
to update-globals
  ...
  set percent-unhappy-blue count turtles with [color = blue and not happy?] / count turtles with [color = blue] * 100
...
"pen-1" 1.0 0 -2674135 true "" "plot percent-unhappy-red"
"pen-2" 1.0 0 -13345367 true "" "plot percent-unhappy-blue"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot moranI"

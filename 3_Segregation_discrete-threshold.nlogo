...
to update-turtles
  ask turtles [
    ...
    ; determine happiness threshold based on neighbor count
    let threshold 0
    if total-nearby = 0 [ set threshold 1 ]
    if total-nearby = 1 [ set threshold 1 ]
    if total-nearby = 2 [ set threshold 1 ]
    if member? total-nearby [3 4 5] [ set threshold 2 ]
    if member? total-nearby [6 7 8] [ set threshold 5 ]

    ; set happiness
    set happy? similar-nearby >= threshold
